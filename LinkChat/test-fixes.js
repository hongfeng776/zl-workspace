...
async function runTests() {
  ...
  const testPhone = '150' + Math.floor(Math.random() * 100000000).toString().padStart(8, '0');
  const testPhone2 = '151' + Math.floor(Math.random() * 100000000).toString().padStart(8, '0');
  ...
  try {
    ...
    const changePwdCodeRes = await request('/auth/send-code', 'POST', { phone: testPhone, type: 'change_password' });
    const changePwdCode = changePwdCodeRes.body.code;
    console.log('  修改密码验证码:', changePwdCode);
    ...
    console.log('========================================');
    console.log('\n修复总结：');
    console.log('  1. 注册验证码接收正常 - 开发环境下验证码在响应中返回');
